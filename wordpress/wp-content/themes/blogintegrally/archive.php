	...
	if ( have_posts() ) :
		while ( have_posts() ) :
			...
			<article class="post">
				...
				<a href="<?php the_permalink(); ?>" class="post__link" aria-label="<?php esc_html_e( 'Lire la suite', 'blog-integrally' ); ?> : <?php the_title(); ?>"><?php esc_html_e( 'Lire la suite', 'blog-integrally' ); ?></a>
			</article>
					<?php
		endwhile;
	else :
